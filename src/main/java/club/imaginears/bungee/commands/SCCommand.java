...
public class SCCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (Permissions.checkPermissionMsg(p, "core.staffchat")) {
                if (args.length > 0) {
                    String message = "";
                    ...
                    for (ProxiedPlayer all : Main.getInstance().getProxy().getPlayers()) {
                        if (all.hasPermission("core.staffchat")) {
                            TextComponent msg = new TextComponent(new TextComponent(Chat.sendColorFree("&cSTAFF &7» &a(&b" + p.getServer().getInfo().getName() + "&a) " + Permissions.getRankPrefix(p) + " &7" + p.getName() + ChatColor.GRAY + ":" + message)));
                            msg.setColor(ChatColor.GRAY);
                            all.sendMessage(msg);
                        }
                    }
                } else {
                    Chat.sendError(p, Chat.ChatErrors.ARGCOUNT, "/sc (message)");
                }
            }
        }
